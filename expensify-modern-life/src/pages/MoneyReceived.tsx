...
export default function MoneyReceived() {
  const [receivedMoney, setReceivedMoney] = useState<Entry[]>([]);
  ...
  const fetchEntries = async (uid: number) => {
    setIsLoading(true);
    try {
      const res = await fetch(`http://localhost:8000/api/money-received`, {
        headers: {
      ...
      setReceivedMoney([]); // Set empty array on error
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/money-received/${deleteConfirm.entry.id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      setReceivedMoney(receivedMoney.filter((entry) => entry.id !== deleteConfirm.entry!.id));
      hideDeleteConfirm();
    } catch (err) {
  ...
  const totalReceived = Array.isArray(receivedMoney) 
    ? receivedMoney.reduce((sum, entry) => sum + Number(entry.amount || 0), 0)
    : 0;
    ...
    <Layout>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Total Received: ₹{totalReceived.toFixed(2)}</CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? (
